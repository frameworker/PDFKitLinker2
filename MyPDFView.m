	...
	if ([(MyWindowController *)[[self window] windowController] linkMode] == kLinkEditMode)
	{
		...
		if (allAnnotations)
		{
			...
			if (foundActive)
			{
				...
				// Draw resize handle.
				NSRectFill(NSIntegralRect([self resizeThumbForRect:bounds rotation:(int)[pdfPage rotation]]));
			}
		}
	}
}
...
- (void) mouseDown: (NSEvent *) theEvent
{
	...
	if ([(MyWindowController *)[[self window] windowController] linkMode] == kLinkEditMode)
	{
		...
		numAnnotations = (int)[annotations count];
        ...
        _activeAnnotation = (PDFAnnotation *)newActiveAnnotation;
		...
		if (_activeAnnotation == NULL) // Hit non-link annotation.
		{
			[super mouseDown: theEvent];
		}
		else
		{
			...
			_resizing = NSPointInRect(pagePoint, [self resizeThumbForRect:_wasBounds rotation:(int)[_activePage rotation]]);
		}
	}
	else
	{
