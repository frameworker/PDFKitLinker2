...
- (void) windowDidLoad
{
	...
	// Create PDFDocument.
    pdfDoc = [[PDFDocument alloc] initWithURL: [[self document] fileURL]]; // JMN Fix
	...
	if ((([_pdfView displayMode] & 0x01) == 0x01) && ([[_pdfView document] pageCount] > 1))
        visibleScreen.size.width -= [NSScroller scrollerWidthForControlSize:NSControlSizeRegular scrollerStyle: NSScrollerStyleOverlay];
...
- (void) setupDocumentNotifications
{
	...
	// Delegate.
	[[_pdfView document] setDelegate: (id)self]; // JMN Voodoo cast to id!
}
...
- (void) updateOutlineSelection
{
	...
	for (i = 0; i < numRows; i++)
	{
		...
		if ([[_pdfView document] indexForPage: [[outlineItem destination] page]] == pageIndex)
		{
            ...
            NSIndexSet *indexes = [[NSIndexSet alloc] initWithIndex: 0];    // row is NSUinteger
		...
		else if ([[_pdfView document] indexForPage: [[outlineItem destination] page]] > pageIndex)
		{
            ...
            {
                NSIndexSet *indexes = [[NSIndexSet alloc] initWithIndex: i - 1]; // row is nsuinteger
                [_outlineView selectRowIndexes:indexes byExtendingSelection:NO];
            }
			_ignoreNotification = NO;
			break;
...
- (void) setupToolbarForWindow: (NSWindow *) window
{
	...
	// We are the delegate
	[toolbar setDelegate: (id)self];  // JMN Voodoo cast to id!
...
- (void) saveDocumentAs: (id) sender
{
    ...
    NSArray *allowedFileTypes = [NSArray arrayWithObjects:@"pdf", nil]; // JMN 07-03-2018
...
////[[self window] beginSheet:_saveWindow completionHandler:nil];
...
////didEndSelector: @selector(goToPagePanelDidEnd: returnCode: contextInfo:) contextInfo: NULL];
	[[self window] beginSheet:_pageNumberPanel completionHandler:nil];
}
...
- (void) updateViewMode: (NSNotification *) notification
{
	switch ([_pdfView displayMode])
	{
        ...
        case kPDFDisplayTwoUpContinuous:        // JMN addition
        NSLog(@"kPDFDisplayTwoUpContinuous view mode encountered!");
        [_viewModeView setSelectedSegment: 2];  // HALP!!! ??? ###
        break;
...
{
    PDFAnnotation   *activeLink;

	if (_edited)
	{
        PDFAnnotation   *wasActiveLink;
...
- (void) doNewLink: (id) sender
{
    ...
    newLink = [[PDFAnnotation alloc] initWithBounds:bounds forType:PDFAnnotationSubtypeLink withProperties:nil];
